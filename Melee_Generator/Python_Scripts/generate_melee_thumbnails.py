...
class Match:
    def __init__(self, _title, _event, _round, _player1, _char1, _player2, _char2,
        ...
        self.p1 = _player1
        self.c1 = _char1
        # Per-set costume requested in the VOD line ("Falco:2"), parallel to
...
def stripSkinsFromTitle(title):
    ...
    filename. Both the ':2' and the legacy ' 2' forms go. Only text inside the
...
def splitCharSkins(char_tokens):
    ...
    A VOD line may name a per-set costume after a colon ("Falco:2"). Returns
    (chars, skins) where a costume entry is None when none was given. The legacy
    inline form ("Falco 2") has no colon and is left on the character token for
...
def fileConfigFor(event_file, all_configs):
    ...
    if not name:
        return {}
    for a_key, a_cfg in (all_configs.get(populate_melee_globals.FILE_CONFIG_KEY) or {}).items():
...
def setGlobals(weekly_event, property_settings=None, vod_file=None,
    ...
    try:
        ...
        for _label in sorted(_all_cfgs, key=len, reverse=True):
            if _label == populate_melee_globals.FILE_CONFIG_KEY:
    ...
    for a_match in match_list:
        ...
        for a_list, a_skin_list in [(a_match.c1, a_match.c1_skins), (a_match.c2, a_match.c2_skins)]:
            ...
            for char_index, (a_char, a_skin_request) in enumerate(
                ...
                # The legacy inline costume, "Falco 2" -- kept so VOD files
                # written before the "Falco:2" syntax still resolve. The colon
