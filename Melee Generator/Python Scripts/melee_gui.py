#!/usr/bin/env python3
"""GUI launcher for the Melee Generator toolset."""
...
RENDERS_DIR = ROOT / "Resources" / "Character_Renders" / "Melee Full Renders"
PLAYER_DB_PATH = ROOT / "Resources" / "Player_database.csv"
CHAR_DB_PATH = ROOT / "Resources" / "Character_database.csv"
THUMBNAIL_SCRIPT = ROOT / "Python Scripts" / "generate_melee_thumbnails.py"
...
def save_player_db(headers, players):
    lines = list(headers)
...
def save_char_db(headers, entries):
    lines = list(headers)
...
class MeleeGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Melee Generator")
    ...
    # ------------------------------------------------------------------ #
    def _build_fetch_tab(self, notebook):
        ...
        for cfg in FETCH_EVENTS:
            ...
            has_num = bool(cfg["default_num"] or cfg["slug_template"].count("{n}"))
        ...
        tk.Label(row_slug, text="e.g. tournament/my-event/event/melee-singles",
    ...
    def _fetch_top8(self, cfg, n, link):
        if "{n}" in cfg["slug_template"]:
            name = cfg["name_template"].format(n=n)
            slug = cfg["slug_template"].format(n=n)
        else:
    ...
    # ------------------------------------------------------------------ #
    def _build_thumbnails_tab(self, notebook):
        tab = tk.Frame(notebook)
    ...
    def _generate_thumbnails(self):
        ...
        if not event_name:
            ...
            return
        self._run([
            PYTHON, str(ROOT / "Python Scripts" / "generate_melee_thumbnails.py"),
    ...
    #  Tab: Player Database                                               #
    # ------------------------------------------------------------------ #
    def _build_player_db_tab(self, notebook):
        tab = tk.Frame(notebook)
    ...
    def _populate_char_tree(self, name):
        self._clear_char_tree()
    ...
    def _move_char_entry(self, direction):
        sel = self._char_tree.selection()
        if not sel or not self._db_selected_player:
            return
        idx = int(sel[0])
    ...
    def _show_preview(self, char, alt):
    ...
    # ------------------------------------------------------------------ #
    def _build_char_db_tab(self, notebook):
        tab = tk.Frame(notebook)
