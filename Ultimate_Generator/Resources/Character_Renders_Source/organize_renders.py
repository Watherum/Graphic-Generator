...
Desire is to grab the portraits for each character organize them in the following way:
    Renders
        Ultimate_Square_render
            Character (1)
            ...
            Character (8)
        Ultimate_Body_render
        Wide render
        Ultimate_Diamond_render
        Ultimate_Full_render (incomplete)
...
def organize_from_renders_zip(char_name, new_folder_location):
    ...
    Move the images from the zip structure to the new location
        chara_0 --> Ultimate_Square_render
        chara_1 --> Ultimate_Body_render
        chara_3 --> Wide render
        chara_4 --> Ultimate_Diamond_render
    ...
    for a_file in filenames:
        ...
        if a_file.endswith(".png") and a_file.startswith("chara_") and a_file[-7:-5] == '_0':
            ...
            if a_file.startswith("chara_0_"):  # Char 0 case
                dest_folder = os.path.join(new_folder_location, "Ultimate_Square_render")
                return_mapping.append((source_path, a_file, dest_folder, dest_file))
            elif a_file.startswith("chara_1_"):  # Char 1 case
                dest_folder = os.path.join(new_folder_location, "Ultimate_Body_render")
                return_mapping.append((source_path, a_file, dest_folder, dest_file))
                ...
                dest_folder = os.path.join(new_folder_location, "Ultimate_Diamond_render")
                return_mapping.append((source_path, a_file, dest_folder, dest_file))
            ...
            dest_folder = os.path.join(new_folder_location, "Ultimate_Full_render")
            return_mapping.append((source_path, a_file, dest_folder, dest_file))
            # end of main case
        # end of loop
    return return_mapping
