...
def fileConfigFor(event_file, all_configs):
    ...
    if not name:
        return {}
    for a_key, a_cfg in (all_configs.get(populate_ultimate_globals.FILE_CONFIG_KEY) or {}).items():
        if str(a_key).strip().lower() == name:
            return a_cfg
    return {}


def setGlobals(weekly_event, property_settings=None, vod_file=None,
               config_file=None):
    """
    ...
    if weekly_event.startswith('Quarantainment'):
        global_properties = populate_ultimate_globals.setGlobalsQuarantainment(weekly_event)
    elif weekly_event.startswith('Students x Treehouse'):
        global_properties = populate_ultimate_globals.setGlobalsSxT(weekly_event)
    elif weekly_event.startswith('Fro Fridays'):
        global_properties = populate_ultimate_globals.setGlobalsFro(weekly_event)
    elif weekly_event.startswith('AWG Just Tech It'):
        global_properties = populate_ultimate_globals.setGlobalsJustTechIt(weekly_event)
    elif weekly_event.startswith('Immortal Fight Night'):
        global_properties = populate_ultimate_globals.setGlobalsIFN(weekly_event) 
    elif weekly_event.startswith('NYS HS Esports Showcase'):
        global_properties = populate_ultimate_globals.setGlobalsIFN(weekly_event) 
    elif weekly_event.startswith('Clip It'):
        global_properties = populate_ultimate_globals.setGlobalsClipIt(weekly_event) 
    elif weekly_event.startswith('CR Clash'):
        global_properties = populate_ultimate_globals.setGlobalsCRClash(weekly_event)
    elif weekly_event.startswith('CR Arcadian'):
        global_properties = populate_ultimate_globals.setGlobalsCRArcadian(weekly_event)
    elif weekly_event.startswith('AWG'):
        global_properties = populate_ultimate_globals.setGlobalsAWG(weekly_event)
    elif weekly_event.startswith('C2C Finale'):
        global_properties = populate_ultimate_globals.setGlobalsC2C(weekly_event)
    elif weekly_event.startswith('Catman'):
        global_properties = populate_ultimate_globals.setGlobalsCatman(weekly_event)
    elif weekly_event.startswith('IzAw Sub') or weekly_event.startswith('Big Forhead Plays'):
        global_properties = populate_ultimate_globals.setGlobalsIzAw(weekly_event)
    else:
        global_properties = populate_ultimate_globals.set_default_properties(weekly_event)
    # A -v override names the file this run actually reads, so it has to be
    # in place before the per-file config below is looked up.
    if vod_file:
        global_properties['event_file'] = vod_file
    # Apply per-event JSON config overrides (from ultimate_event_configs.json)
    try:
        import json as _json
        _cfg_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "ultimate_event_configs.json")
        with open(_cfg_path, encoding="utf-8") as _f:
            _all_cfgs = _json.load(_f)
        for _label in sorted(_all_cfgs, key=len, reverse=True):
            if _label == populate_ultimate_globals.FILE_CONFIG_KEY:
        ...
        # thumbnail is not a singles one -- and the names file tells them apart.
        global_properties.update(fileConfigFor(
            config_file or global_properties.get('event_file', ''), _all_cfgs))
    except Exception:
        pass
    # return properties
...
def createMatches(match_lines, log_file=None, event_name=None, event_short_name=None,
    ...
    for a_line in match_lines:
        ...
        # A doubles entrant is a team. start.gg spells it "A / B", which cannot
        # survive into the title -- the title becomes the output filename, and
        # '/' is a path separator on Windows. Rewrite it to the comma form here
        # so files written before that change still generate.
    ...
    for a_match in match_list:
        ...
        for a_list, a_skin_list in [(a_match.c1, a_match.c1_skins), (a_match.c2, a_match.c2_skins)]:
            ...
            for char_index, (a_char, a_skin_request) in enumerate(
                    zip(a_list, a_skin_list)):
                ...
                # fourth still reports its missing player, character or costume.
...
def createRoundImages(match_list, background, foreground, only_one=False):
    ...
    for a_match in match_list:
        ...
        # Sizes rather than fonts: each label's font is built below at whatever
        # size actually fits the canvas, which a doubles team name often is not.
...
def main(argv):
    ...
    try:
        ...
        print('generate_ultimate_thumbnails.py -e <event>, -p <property_file>, -o <output_file>, -v <vod_file>, -c <config_file>')
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print('generate_ultimate_thumbnails.py -e <event (can have a number)>, -p <property_file>, -o <output_file>, -v <vod_file>, -c <config_file for per-file config lookup>')
            sys.exit()
        elif opt in ("-e", "--event"):
            event_title = arg
    ...
    _character_database = populate_ultimate_globals.readCharDatabase(character_database_location)
    global _player_database
    player_database_location = os.path.join('Resources', 'Player_Database.csv')
    _player_database = populate_ultimate_globals.readPlayerDatabase(player_database_location, char_database=_character_database)
    # 1. Read in the names file to get event, round, names, characters information
