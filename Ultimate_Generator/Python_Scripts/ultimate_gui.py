#!/usr/bin/env python3
"""PySide6 GUI for the Ultimate_Generator toolset.

Launched via ``Launch_Ultimate_GUI.vbs``. This replaced the original Tkinter GUI,
...
(e.g. ``Mario (5).png``) rather than named skins, and the character database is a
...
THUMBNAIL_SCRIPT = ROOT / "Python_Scripts" / "generate_ultimate_thumbnails.py"
FULL_RENDERS_DIR = ROOT / "Resources" / "Character_Renders" / "Ultimate_Full_render"
...
try:
    import populate_ultimate_globals as _pg
except ImportError:
    _pg = None
...
def _base_event_props(series: str) -> dict:
    """Default property dict for a series, via the populate_ultimate_globals
...
def _make_check_icon() -> str:
    ...
    import tempfile
    path = Path(tempfile.gettempdir()) / "ultimate_qt_check.png"
...
def _thumbnail_generator():
    ...
    if _gen_module is None:
        import generate_ultimate_thumbnails as _m
        _gen_module = _m
    return _gen_module
...
#  Main window                                                                 #
# --------------------------------------------------------------------------- #
class UltimateWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Ultimate Generator Powered By Watherum")
    ...
    #     generate_ultimate_thumbnails.py as per-event property overrides) ---
    ...
    def _preview_sample_line(self, event_name: str):
        ...
        return (f"{event_name} - Grand Final - Player One ({', '.join(p1)}) Vs "
                f"Player Two ({', '.join(p2)}) - SSBU", event_name, "")

    def _build_preview_image(self):
        """Render one thumbnail from the current form values."""
        if _pg is None:
            raise RuntimeError("populate_ultimate_globals could not be imported")
    ...
    def _generate_thumbnails(self):
        ...
        if not event_name:
            ...
            return
        cmd = [PYTHON, str(ROOT / "Python_Scripts" / "generate_ultimate_thumbnails.py"),
    ...
    def _build_update_tab(self):
        ...
        box = QtWidgets.QGroupBox("Update Ultimate Generator")
        v = QtWidgets.QVBoxLayout(box)
        v.addWidget(_muted(
            "Check GitHub for a newer version of this generator and apply it. "
            "Only the Ultimate_Generator folder is updated — your player "
