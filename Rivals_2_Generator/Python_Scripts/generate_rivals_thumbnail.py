...
_character_database = {}
_player_database = {}
_char_default_skin = {}  # char_upper -> default wiki filename stem (T_{Abbrev}_Default_Neutral_CSP)
    ...
    for a_match in match_list:
        ...
        for a_list in [a_match.c1, a_match.c2]:
            ...
            for a_char in a_list:
                ...
                # Default to Default_Neutral skin; overridden by player database lookup below
                char_file = _char_default_skin.get(a_char.upper(), a_char)
                # Lookup character in player database for alt costume
                if p1_flag:
...
def createCharacterWindow(char_list, win_size, right_bool=False, single_bool=False, border_bool=True, only_one=False):
    ...
    for a_render, a_resize in zip(render_list, resized_char_list):
        ...
        for a_char in char_list:
            ...
            # Mirror right-side characters so they face left (toward center)
            if right_bool:
                a_char_image = a_char_image.transpose(Image.FLIP_LEFT_RIGHT)
            # Fit to window for specific cases
            if a_render == "Rivals_2_Full_Renders":
...
def main(argv):
    ...
    # Build default skin map: scan all wiki filenames to find each character's Default_Neutral skin
    global _char_default_skin
    import re as _re
