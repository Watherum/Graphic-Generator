...
# The VOD editor's skin preview and the gap after it. The count label in the
# filter bar borrows both, which is what puts "Filter:" directly under "File:".
_VOD_PREVIEW_PX = 200
_VOD_PREVIEW_GAP = 16
...
def _normalize_startgg_slug(value: str) -> str:
    ...
    ("…/event/rivals-2-singles/brackets/1/2"), which the API rejects — keep
...
def _startgg_slug_problem(value: str) -> str:
    ...
    A tournament runs several events (Rivals 2 singles, doubles, Ultimate...)
    ...
    if "/event/" not in slug:
        return ("This is the tournament, not one of its events. A tournament "
                "holds several events; open the one you want (e.g. Rivals 2 "
...
class RivalsWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        # Players / Add Missing Skins buttons write into it. Moved after the
    ...
    def _event_name_for_file(name: str) -> str:
        ...
        if name.lower().endswith(RivalsWindow._NAMES_SUFFIX.lower()):
            return name[:-len(RivalsWindow._NAMES_SUFFIX)]
    ...
    def _build_vod_editor(self, parent_layout, expanding=False):
        ...
        # Second row: the character/skin pickers and the two database buttons.
        # Splitting them off the file selector keeps each row short enough to
        # read left-to-right instead of one long strip of unrelated controls.
        row2 = QtWidgets.QHBoxLayout()
        ...
        row2.addWidget(crb)
        row2.addWidget(QtWidgets.QLabel("Skin:"))
        ...
        row2.addWidget(cc)
        row2.addSpacing(20)
        # Players first: a skin has nowhere to go until its player has a row,
        # which is the order the two buttons are used in.
        ...
        # at this size it spans the file and picker rows instead of stretching
        # either one.
        ...
        # The stretch goes between the rows, not after them: the preview is 200px
        # tall whatever these rows do, so parking the pickers at the bottom of
        # that column puts them right above the filter bar -- the two rows that
        # act on the table sit together -- instead of leaving dead space there.
        ...
        # Only a user's own pick renames the event -- a programmatic refresh
        # must not overwrite the event name it was called to follow.
    ...
    def _top8_char_signature(text: str) -> tuple:
        """The character/skin fields of every placement line, in order.

        A placement line is `place,name,sponsor,char[:skin][,char2...]`, so
    ...
    def _on_char_tree_check(self, item, column):
        ...
        character has a second skin to tick. So the row index is taken here and
        the rebuild is deferred to the next event-loop turn, by which point Qt
    ...
    def _apply_char_tree_check(self, player: str | None, idx: int, checked: bool):
